...
from src.features.resolutions.services.resolution_service import pause_resolution
...
@router.post(
    "/{resolution_id}/pause",
    operation_id="pauseResolution",
    response_model=SuccessResponse[ResolutionSummarySchema],
    responses={400: {"model": ErrorResponse}, 404: {"model": ErrorResponse}},
)
async def pause_resolution_route(
    resolution_id: UUID,
    current_user: Annotated[CurrentUser, Depends(get_current_user)],
    db: Annotated[AsyncSession, Depends(get_db)],
):
    resolution = await pause_resolution(db, resolution_id=resolution_id, user_id=current_user.id)
