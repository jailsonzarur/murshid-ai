...
async def get_exam_by_id(db: AsyncSession, exam_id: UUID) -> ExamModel | None:
    return await db.get(ExamModel, exam_id)


async def add_exam(db: AsyncSession, exam: ExamModel) -> ExamModel:
    db.add(exam)
    await db.flush()
    return exam


def add_exam_document(db: AsyncSession, document: ExamDocumentModel) -> None:
    db.add(document)


async def remove_exam(db: AsyncSession, exam: ExamModel) -> None:
