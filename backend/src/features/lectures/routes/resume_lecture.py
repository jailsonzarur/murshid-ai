...
from src.features.lectures.services.lecture_service import resume_lecture
...
@router.post(
    "/{lecture_id}/resume",
    operation_id="resumeLecture",
    response_model=SuccessResponse[LectureSummarySchema],
    responses={400: {"model": ErrorResponse}, 404: {"model": ErrorResponse}},
)
async def resume_lecture_route(
    lecture_id: UUID,
    current_user: Annotated[CurrentUser, Depends(get_current_user)],
    db: Annotated[AsyncSession, Depends(get_db)],
):
    lecture = await resume_lecture(db, lecture_id=lecture_id, user_id=current_user.id)
